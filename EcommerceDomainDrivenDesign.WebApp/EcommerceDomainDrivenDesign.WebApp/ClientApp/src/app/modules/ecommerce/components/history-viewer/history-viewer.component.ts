...
export class HistoryViewerComponent implements OnInit {
  ...
  getCustomerStoryData() {
    this.historyViewerService
      .getCustomerEventHistory(this.aggregateId)
      .subscribe(
        (result: any) => {
  ...
  getObjectKeys<T>(obj: T): HistoryDataRow[] {
    ...
    if (obj) {
      ...
      objectKeys.forEach((key) => {
        ...
        if (key === 'action') record.position = 0;
        else record.position = index;
      ...
      if (action.length > 0) action[0].position = keyList.length + 1;
    }
    return keyList;
  }
