...
export class AuthService extends RestService {
  ...
  constructor(
    ...
    var storedCustomer = JSON.parse(
      this.localStorageService.getValueByKey('currentCustomer'),
    );
  ...
  login(email: string, password: string) {
    return this.post('customers/login', { email, password }).pipe(
      map((customer) => {
        ...
        if (customer && customer.data.token) {
          ...
          this.notificationService.showError(
            customer.data.validationResult.errors[0].errorMessage,
          );
        }
